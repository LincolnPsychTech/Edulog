...
if isempty(varargin)
    varargin = fieldnames(data)';
    varargin(strcmp(varargin, "Time") | strcmp(varargin, "Concern")) = [];
end
...
for L = 1:length(loggers)
    ...
    for E = 1:length(evtype) % For each kind of event...
        ...
        ev{L, E} = line(... % Plot events
            ...
            'Color', ecol(E,:), ... % Set colour to iterate
            'LineWidth', 2 ... % Make lines 2 thick
        );
    end
...
coLg = line(ax{L}, ... % Create archetype line for concern at 0 0
    [0 0], ... % X data is 0s
    ax{L}.YLim', ... % Y data is just axis limits
    'Color', 'r', ... % Set colour to red
    'LineStyle', ':' ... % Make lines dashed
...
lnLg = line(ax{L}, ... % Create archetype data line at 0 0
        [0 0], ... % X data is 0s
        ax{L}.YLim', ... % Y data is just axis limits
        'Color', [42, 107, 211]./255, ... % Set colour to royal blue
        'LineWidth', 2 ... % Make lines 2 thick
