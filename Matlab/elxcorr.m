function [r, lag] = elxcorr(data, varargin)
...
for c = varargin % For each corrlate...
    ts.(c{:}) = [data([data.(c{:})]).Time]; % Extract timestamps of events
    if length(ts.(c{:})) <= 1
        rmfield(ts, c{:});
        varargin(strcmp(varargin, c{:})) = [];
    end
end

corrmat = [... % Create row of vertical pairs containing every combination of two fields
    reshape(meshgrid(varargin), 1, []); ...
	reshape(meshgrid(varargin)', 1, []) ...
    ];

r = table(... % Create a table to store r values, which has...
    'Size', repmat(length(varargin), 1, 2), ... % ...the same number of rows and columns as there are fields
    'VariableTypes', repmat("double", 1, length(varargin)), ... % ...contents of type "double"
    'VariableNames', varargin, ... % ...the strings contained in "correlates" as variable names
    'RowNames', varargin ... % ...the strings contained in "correlates" as row names
    );
