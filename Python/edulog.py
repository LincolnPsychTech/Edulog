...
def sps(data):
    ...
    sps = int(round(1/numpy.nanmean(diffs))); # Calculate average samples per second
    return sps
...
def gsrsplit(data):
    ...
    SPS = sps(data) # Estimate the number of samples per second
...
def scr(data, method):
    ...
    if (method == "median") | (method == "med") | (method == 'median') | (method == 'med'): # If method is "median"...
        data.insert( 
                len(data.columns), "SCR", 
                (data.Phasic < q2 - 2*sd) | (data.Phasic > q2 + 2*sd) 
                ) # Find points where phasic response was +-2sd of q2
    elif (method == "quartiles") | (method == "q") | (method == 'quartiles') | (method == 'q'): # If method is "quartiles"...
        data.insert( 
                len(data.columns), "SCR", 
                (data.Phasic < q1 - 2*sd) | (data.Phasic > q3 + 2*sd) 
                ) # Find points where phasic response was 2sd above / below inter-quartile range
    else:
...
def beat(data):
    ...
    pha = []; # Create blank list to store phasic
    for n in range(len(data)): # For each data point
        if n <= 8*SPS: # for the first 2 seconds...
            ton.append( numpy.nanmean(data.EKG[0:n+8*SPS]) ); # Take the mean of all points up the this one
        elif n >= len(data)-8*SPS: # for the last 2 seconds...
            ton.append( numpy.nanmean(data.EKG[n-8*SPS:len(data)]) ); # Take the mean of all points from this one
        else: # for the rest...
            ton.append( numpy.nanmean(data.EKG[n-8*SPS:n+8*SPS]) ); # Take the mean of all points 2s either side of the data
        pha.append(data.EKG[n] - ton[n]) # Subtract the tonic value from the original value to get the phasic signal
